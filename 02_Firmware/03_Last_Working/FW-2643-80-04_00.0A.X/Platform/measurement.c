...
void store_mat_thick_raw(void){ // Probe Thickness calculation
    ...
    BYTE3(mat_thick_raw) = mb_rx_buff[5];
	BYTE2(mat_thick_raw) = mb_rx_buff[6];
	BYTE1(mat_thick_raw) = mb_rx_buff[3];
	BYTE0(mat_thick_raw) = mb_rx_buff[4];
    mat_thick_raw = mat_thick_raw * 1000;
...
void store_mat_thick_CT(void){  // Temperature compensated Probe Thickness calculation
    //- you need to swap the word order of the result value before converting from IEEE 32bit Float
    BYTE3(mat_thick_TC) = mb_rx_buff[9];
	BYTE2(mat_thick_TC) = mb_rx_buff[10];
	BYTE1(mat_thick_TC) = mb_rx_buff[7];
	BYTE0(mat_thick_TC) = mb_rx_buff[8];
    mat_thick_TC = mat_thick_TC * 1000;
}

void store_mat_temp(void){  // Temperature
    //- you need to swap the word order of the result value before converting from IEEE 32bit Float
    BYTE3(mat_temp) = mb_rx_buff[13];
	BYTE2(mat_temp) = mb_rx_buff[14];
	BYTE1(mat_temp) = mb_rx_buff[11];
	BYTE0(mat_temp) = mb_rx_buff[12];
}
    ...
    switch(mes_para_stat){
        ...
        case MB_PARA_VAL_03:  //
            if(mb_comm_stat == MB_RX_FINSHED){
                ...
                //Save received Measurement Mode value
//                BYTE0(MB_Para.mb_uiMeasurement_mode) = mb_rx_buff[44];
//                BYTE1(MB_Para.mb_uiMeasurement_mode) = mb_rx_buff[43];
...
//        MB_Para.mb_fTempCompRefTemp=0;
//        MB_Para.mb_fTempCompFactor=0;
          strt_measurement(3);
        }else if(mes_err_stat== MB_ERROR_NOREPLY){
            if(!remeasure){
                tx_counter=0;
